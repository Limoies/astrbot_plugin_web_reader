    ...
    async def _analyze_with_llm(
        ...
        format_prompts = {
            "summary": (
                ...
                "1. 用中文回答，内容全面详尽\n"
                "2. 包含文章的核心主题、主要观点、关键数据、论证过程\n"
                "3. 按逻辑分层呈现，使用多级标题组织内容结构\n"
                "4. 保留重要的事实、数据、引用和结论\n"
                "5. 格式为Markdown，确保条理清晰、信息完整"
            ),
            "bullets": (
                "请将以下内容提炼为要点列表，要求：\n"
                "1. 用中文回答\n"
            ...
            "organize": (
                "请对以下网页内容进行结构整理，注意：\n"
                "1. **这不是总结任务！这是整理任务！**\n"
                "2. **必须输出完整的原文，逐字逐句保留，不得删减、概括或改写任何内容**\n"
                "3. 根据原文的逻辑划分段落，添加合适的小标题和标题层级\n"
                "4. 用 Markdown 标题（# ## ###）来组织结构层次\n"
                "5. 识别并标注原文中的列表、引用等特殊格式\n"
                "6. 整理后的内容应当比原文更长（因为加了标题和结构标记），信息量不变"
            ),
            "csv": (
                "请将以下内容整理为CSV格式，要求：\n"
                "1. 第一行为列名\n"
